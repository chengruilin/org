...
**** DONE Android JNI 接口的实现
     CLOSED: [2018-10-26 五 18:11]
     WXBridge 中对 execJS 方法的 JNI 接口定义:
     #+NAME: WXBridge.java
     #+BEGIN_SRC java
...
***** -> ExecJS()
      ...
      #+END_SRC
      然而并不是个头,在这里通过 IPC 的方式发送到另一个进程去了.

***** -> CoreSideInMultiProcess => sender_->send()
...
***** -> IPCSender => send()
      IPC 的功能是 Android 特有的,用共享内存的方式来实现 IPC 通信.
      #+NAME: IPCSender.cpp
      #+BEGIN_SRC c++
...
**** DOING Android IPCListener
     execJS 方法调用的最后是将 JS 调用当成了一个数据指令通过 IPC 的方式发送出去的,现在我们来看下发送之后的消息是怎么处理的.
***** 初始化 IPC 消息监听

      初始化 Framework 的时候会设置 ~ScriptBridge~ 对象(~ScriptBridgeInMultiProcess~):
      #+NAME: wx_bridge.cpp
      #+BEGIN_SRC c++
        static jint InitFramework(JNIEnv* env, jobject object, jstring script, jobject params) {
          // other code...
...
***** -> WeexJSConnection => start()
      
      先来看下 ~pthread_create~ 方法, 它主要用来创建线程:
      #+BEGIN_SRC c++
        #include <pthread.h>
        /*
...
**** TODO Android IPCHandler


***** -> IPCHandler => handler()
      创建 IPCHandler 对象: [[weex_ipc_handler_create][WeexJSConnection => newIPCServer()]]  方法中创建了一个 IPCHandler 实例.
